...
export class RewardHistory {
    ...
    @ApiProperty()
    @Column({ comment: '1=current account is collator, default is 0', default: 0 })
    isCollator: number;

    @ApiProperty()
    @Column({ comment: '1=current account is nominator, default is 0', default: 0 })
    isNominator: number;

    @ApiProperty()
    @Column({ comment: 'the blocknumber which the reward will be transfered', type: 'bigint' })
    issueBlock: number;
